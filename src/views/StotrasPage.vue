<template>
  <div class="max-w-reading mx-auto px-8 py-16">
    <div class="mb-16">
      <p class="text-xs uppercase tracking-widest text-gold mb-3">Sacred Hymns</p>
      <h1 class="text-4xl font-serif text-charcoal mb-4">Stotras</h1>
      <p class="text-text-secondary">Sanskrit hymns and prayers for divine blessings.</p>
    </div>
      ...
      >
        <div class="flex items-start justify-between gap-4">
          <div class="flex-1">
            ...
            <p class="text-text-secondary text-sm mb-3">{{ stotra.deity }} · {{ stotra.language }}</p>
            <p class="text-text-secondary text-sm leading-relaxed">{{ stotra.description }}</p>
            <div class="flex flex-wrap gap-2 mt-3">
              <span v-for="tag in (stotra.tags || []).slice(0,3)" :key="tag" class="tag-minimal">{{ tag }}</span>
            </div>
          </div>
          <span class="text-gold text-lg flex-shrink-0 mt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">→</span>
        </div>
      </div>
      <p v-if="!filtered.length" class="text-text-secondary py-12 text-center">No stotras found.</p>
    </div>
  </div>
</template>
