  ...
  <div>
    <div class="bg-parchment border-b border-border-subtle">
      <div class="max-w-reading mx-auto px-6 md:px-8 py-14">
        <p class="section-label mb-3">{{ t('bhajans.label') }}</p>
        <h1 class="text-4xl md:text-5xl font-serif text-charcoal mb-4">{{ t('bhajans.title') }}</h1>
        <p class="text-text-secondary text-lg max-w-xl">{{ t('bhajans.subtitle') }}</p>
      </div>
    </div>
    ...
    <div class="max-w-reading mx-auto px-6 md:px-8 py-10">
      <div class="flex flex-col sm:flex-row gap-3 mb-10">
        <div class="relative flex-1">
          ...
          <input v-model="searchQuery" :placeholder="t('bhajans.searchPlaceholder')" class="w-full pl-11 pr-4 py-3.5 border border-border-subtle focus:outline-none focus:border-gold transition-colors text-charcoal bg-white text-sm">
        </div>
        <select v-model="selectedDeity" class="px-4 py-3.5 border border-border-subtle focus:outline-none focus:border-gold transition-colors text-text-secondary bg-white text-sm min-w-40">
          <option value="">{{ t('bhajans.allDeities') }}</option>
      ...
      <p v-if="!loading" class="text-xs text-text-muted uppercase tracking-widest mb-6">
        {{ filtered.length }} {{ filtered.length === 1 ? t('bhajans.foundOne') : t('bhajans.found') }}
      </p>
      ...
      <div class="bg-white border border-border-subtle overflow-hidden">
        ...
        >
          <div class="flex items-start gap-4">
            <div class="w-10 h-10 flex items-center justify-center flex-shrink-0 rounded-sm text-lg" style="background:#F0EEFF;">🎵</div>
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-4">
                <div class="flex-1">
                  <h3 class="text-lg font-serif text-charcoal mb-1 group-hover:text-gold transition-colors duration-200">{{ bhajan.title }}</h3>
                  <p class="text-text-secondary text-sm mb-2">{{ bhajan.deity }} · {{ bhajan.language }}</p>
                  <p class="text-text-secondary text-sm leading-relaxed line-clamp-2">{{ bhajan.description }}</p>
                  <div class="flex flex-wrap gap-1.5 mt-3">
                    <span v-for="tag in (bhajan.tags || []).slice(0,3)" :key="tag" class="tag-devotional">{{ tag }}</span>
                  </div>
                </div>
        ...
        <div v-if="!loading && !filtered.length" class="px-6 py-16 text-center">
          ...
          <p class="text-text-secondary">{{ t('bhajans.notFound') }}</p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  name: 'BhajansPage',
  mixins: [langMixin],
  data() { return { searchQuery: '', selectedDeity: '', bhajans, loading: true }; },
  mounted() { setTimeout(() => { this.loading = false; }, 500); },
  computed: {
    deityOptions() { return [...new Set(this.bhajans.map(b => b.deity))]; },
    filtered() {
      return this.bhajans.filter(b => {
        const q = this.searchQuery.toLowerCase();
        return (b.title.toLowerCase().includes(q) || (b.description || '').toLowerCase().includes(q))
          && (!this.selectedDeity || b.deity === this.selectedDeity);
      });
    }
  }
};
